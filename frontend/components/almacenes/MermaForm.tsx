...
import { useCrearMermaAlmacenApiV1MermasPost } from "@/lib/api/generated/endpoints/ajustes-mermas-y-devoluciones/ajustes-mermas-y-devoluciones";
...
export function MermaForm({
  ...
  const crear = useCrearMermaAlmacenApiV1MermasPost();
  ...
  if (almacenes.length === 0 || productos.length === 0) {
    return (
      <p className="rounded-md bg-warning/10 px-4 py-3 text-sm text-warning">
        Se necesita al menos un almacén y un producto para registrar una merma.
      </p>
    );
  }
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-1 gap-2 sm:grid-cols-4 sm:items-end">
        ...
        <div className="space-y-1">
          ...
          <input
            ...
            required
            maxLength={255}
            placeholder="VENCIMIENTO, DETERIORO, ROTURA..."
            className="w-full rounded-md border border-border/30 px-2 py-1.5 text-sm focus:border-primary focus:outline-none"
          />
        </div>
      </div>
      ...
      >
        {crear.isPending ? "Registrando..." : "Registrar merma"}
      </button>
    </form>
  );
}
