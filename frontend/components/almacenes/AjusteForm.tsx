...
import { useCrearAjusteInventarioApiV1AjustesInventarioPost } from "@/lib/api/generated/endpoints/ajustes-mermas-y-devoluciones/ajustes-mermas-y-devoluciones";
...
export function AjusteForm({
  ...
  const crear = useCrearAjusteInventarioApiV1AjustesInventarioPost();
  ...
  async function handleSubmit(event: React.FormEvent) {
    ...
    const response = await crear.mutateAsync({
      data: {
        ...
        cantidad_ajuste: Number(cantidadAjuste),
        motivo,
      },
    });
  ...
  if (almacenes.length === 0 || productos.length === 0) {
    return (
      <p className="rounded-md bg-warning/10 px-4 py-3 text-sm text-warning">
        Se necesita al menos un almacén y un producto para registrar un ajuste.
      </p>
    );
  }
  ...
  return (
    ...
    >
      <p className="text-sm font-medium text-foreground">Ajuste de inventario</p>
      {error && (
      ...
      <div className="grid grid-cols-1 gap-2 sm:grid-cols-4 sm:items-end">
        ...
        <div className="space-y-1">
          <label className="text-xs text-text-secondary">Cantidad de ajuste (± )</label>
          <input
            type="number"
            step="0.01"
      ...
      >
        {crear.isPending ? "Registrando..." : "Registrar ajuste"}
      </button>
    </form>
  );
}
